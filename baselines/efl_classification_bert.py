    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text, pattern, target_label) in self.sample(random):
            if (self.is_pre):
                ...
                token_ids, segment_ids = tokenizer.encode(first_text=text, second_text=pattern, maxlen=maxlen)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run EFL-BERT classification for few-shot.')
    # About datasets
    ...
    parser.add_argument("--model_name", type=str, default='bert_base',
                        choices=['bert_large', 'chinese_bert_base', 'chinese_bert_base_ocnli', 'bert_large_mnli'],
