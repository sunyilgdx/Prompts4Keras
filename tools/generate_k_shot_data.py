...
def get_label(task, line):
    ...
    elif task in ["IMDB", "Amazon", "DBpedia"]:
        line = line.strip().split('\t')
        return line[0]
...
def load_datasets(data_dir, tasks):
    ...
    for task in tasks:
        if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
            ...
            splits = ["train", "test"]
            for split in splits:
                data_filename = os.path.join(dirname, f"{split}.txt")
                label_filename = os.path.join(dirname, f"{split}_labels.txt")
                lines = []
                with open(data_filename, "r", encoding='utf-8') as data_f:
                    with open(label_filename, "r", encoding='utf-8') as label_f:
                        for d, l in zip(data_f.readlines(), label_f.readlines()):
                            lines.append("{}\t{}\n".format(l.strip(), d.strip()))
                dataset[split] = lines
            datasets[task] = dataset

        elif task in ["AGNews", "Yahoo"]:
            dataset = {}
            dirname = os.path.join(data_dir, task)
            splits = ["train", "test"]
            for split in splits:
...
def main():
    ...
    parser.add_argument("--task", type=str, nargs="+",
                        default=['SST-2', 'MR', 'CR', 'MPQA', 'Subj', 'MRPC', 'AGNews', 'Yahoo', 'QQP', 'STS-B', 'MNLI',
                                 'SNLI',
                                 'QNLI', 'RTE'],
                        help="Task names")
    ...
    parser.add_argument("--data_dir", type=str, default=r"../datasets/", help="Path to original data")
    parser.add_argument("--output_dir", type=str, default="../datasets/", help="Output path")
    ...
    for seed in args.seed:
        ...
        for task, dataset in datasets.items():
            ...
            elif task in ["Amazon", "IMDB", "DBpedia"]:
