...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_acc >= self.best_val_acc:
            ...
            bert.save_weights_as_checkpoint('nli_roberta.weights')
            cls_encoder.save_weights('cls_nli_roberta_encoder.weights')
            self.best_epoch = epoch + 1
        if (epoch >= epoch_num - 1):
            print("Loading best weights...".format(val_acc))
            cls_encoder.load_weights('cls_nli_roberta_encoder.weights')
...
def main():
    ...
    train_data = dataset.load_data(dataset.train_path, sample_num=6000, is_shuffle=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run NLI fine-tuning to generate NLI-RoBERTa.')
    # About datasets
    parser.add_argument('--n_th_set', type=int, choices=[-1], help="Random Sub Dataset", default=-1)
